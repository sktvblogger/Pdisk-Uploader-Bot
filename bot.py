...
async def start(bot, message):
    await message.reply(
        ...
        "𝐈'𝐦 𝐚 𝐏𝐝𝐢𝐬𝐤 𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐫 𝐛𝐨𝐭. 𝐉𝐮𝐬𝐭 𝐬𝐞𝐧𝐝 𝐦𝐞 𝐥𝐢𝐧𝐤, 𝐟𝐢𝐥𝐞 𝐨𝐫 𝐅𝐮𝐥𝐥 𝐩𝐨𝐬𝐭...\𝐧\𝐧 𝐓𝐡𝐢𝐬 𝐛𝐨𝐭 𝐢𝐬 𝐦𝐚𝐝𝐞 𝐛𝐲 @SLBotsOfficial💖")
...
async def pdisk_up(link):
    ...
    res = requests.get(
        'https://pdisk.pro/api/upload/url?key=' + PDISK_API_KEY + '&url=' + link + '&fld_id=0')
    data = res.json()
    data = dict(data)
    print(data)
    v_id = data['result']['file_code']
    v_url = 'https://www.pdisks.com/share-video?videoid=' + v_id
    return (v_url)
